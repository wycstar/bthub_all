...
if [ `cat /etc/rc.local | grep "#MONGODB_TAG" | wc -l` -eq 0 ];then
	...
	sed -i '$inohup mongod --replSet bt --config /home/data/mongodb/conf/mongod.conf > /dev/null &' /etc/rc.local
	sed -i '$iservice elasticsearch start' /etc/rc.local
fi
sudo service mongod stop
sleep 2
nohup mongod --replSet bt --config $MONGO_HOME/conf/mongod.conf > /dev/null &
sleep 2
...
cd $DIR/mongodb/log
nohup mongo-connector -m localhost:27017 -t localhost:9200 -d elastic2_doc_manager > /dev/null &
reboot
exit
